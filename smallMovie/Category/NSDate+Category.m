...
- (NSString *)minuteDescription
{
    ...
    if ([theDay isEqualToString:currentDay]) {//当天
        ...
        NSString *time = [str substringToIndex:2];
        NSMutableString *multimeStr = [NSMutableString stringWithString:[dateFormatter stringFromDate:self]];
        if ([time intValue] >= 0 && [time intValue] < 6) {
            return [NSString stringWithFormat:@"凌晨 %@", [dateFormatter stringFromDate:self]];
        } else if ([time intValue] >= 6 && [time intValue] < 12){
            return [NSString stringWithFormat:@"上午 %@", [dateFormatter stringFromDate:self]];

        } else if ([time intValue] >= 12 && [time intValue] < 18){
            [multimeStr replaceCharactersInRange:NSMakeRange(0, 2) withString:[NSString stringWithFormat:@"%d",[time intValue]-12]];
            return [NSString stringWithFormat:@"下午 %@", multimeStr];
        } else {
            [multimeStr replaceCharactersInRange:NSMakeRange(0, 2) withString:[NSString stringWithFormat:@"%d",[time intValue]-12]];
            return [NSString stringWithFormat:@"晚上 %@", multimeStr];

        }
    }
    else if ([[dateFormatter dateFromString:currentDay] timeIntervalSinceDate:[dateFormatter dateFromString:theDay]] == 86400) {//昨夜
        [dateFormatter setDateFormat:@"HH:mm"];
        return [NSString stringWithFormat:@"昨夜 %@", [dateFormatter stringFromDate:self]];
    }
...
-(NSString *)formattedTime{
    ...
    if (!hasAMPM) { //24小时制
        if (hour <= 24 && hour >= 0) {
            ...
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"昨夜HH:mm"];
        }else {
