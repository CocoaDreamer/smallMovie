    ...
    if(self = [super init]){
        ...
        for (UIView *obj in self.leftVC.view.subviews) {
            ...
            if ([obj isKindOfClass:[UIImageView class]]) {
                self.leftIconImageView = (UIImageView *)obj;
            }
        }
        self.leftTableview.backgroundColor = [UIColor clearColor];
        self.leftIconImageView.backgroundColor = [UIColor clearColor];
        self.leftTableview.frame = CGRectMake(0, 0, kScreenWidth - kMainPageDistance, kScreenHeight);
        self.leftIconImageView.frame = CGRectMake(0, 0, 70, 70);
        //设置左侧tableview的初始位置和缩放系数
        self.leftTableview.transform = CGAffineTransformMakeScale(kLeftScale, kLeftScale);
        self.leftIconImageView.transform = CGAffineTransformMakeScale(kLeftScale, kLeftScale);
...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    if (needMoveWithTap && (rec.view.frame.origin.x >= 0) && (rec.view.frame.origin.x <= (kScreenWidth - kMainPageDistance)))
    {
        ...
        self.leftIconImageView.center = CGPointMake(leftTabCenterX, kScreenHeight * 0.2);

        self.leftTableview.transform = CGAffineTransformScale(CGAffineTransformIdentity, leftScale,leftScale);
        self.leftIconImageView.transform = CGAffineTransformScale(CGAffineTransformIdentity, leftScale,leftScale);
...
-(void)handeTap:(UITapGestureRecognizer *)tap{
    ...
    if ((!self.closed) && (tap.state == UIGestureRecognizerStateEnded))
    {
        ...
        self.leftIconImageView.transform = CGAffineTransformScale(CGAffineTransformIdentity,kLeftScale,kLeftScale);
...
- (void)openLeftView;
{
    ...
    self.leftIconImageView.center = CGPointMake((kScreenWidth - kMainPageDistance) * 0.5, kScreenHeight * 0.2);

    self.leftTableview.transform = CGAffineTransformScale(CGAffineTransformIdentity,1.0,1.0);
    self.leftIconImageView.transform = CGAffineTransformScale(CGAffineTransformIdentity,1.0,1.0);
